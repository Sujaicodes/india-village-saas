...
async function logRequest(req: AuthRequest, res: Response, next: NextFunction) {
...
// Get all states
app.get('/api/states', authenticateApiKey, logRequest, async (req: Request, res: Response) => {
  try {
    const result = await pool.query(
      'SELECT id, state_code, state_name FROM states ORDER BY state_name'
    );
...
app.get('/api/states/:id/districts', authenticateApiKey, logRequest, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;

    const result = await pool.query(
      `SELECT d.id, d.district_code, d.district_name, s.state_name
       FROM districts d
...
app.get('/api/districts/:id/sub-districts', authenticateApiKey, logRequest, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;

    const result = await pool.query(
      `SELECT sd.id, sd.sub_district_code, sd.sub_district_name, d.district_name
...
app.get('/api/sub-districts/:id/villages', authenticateApiKey, logRequest, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const result = await pool.query(
      `SELECT v.id, v.village_name, v.pincode, v.population, sd.sub_district_name
       FROM villages v
...
// Search villages
app.get('/api/search/villages', authenticateApiKey, logRequest, async (req: Request, res: Response) => {
  try {
...
app.get('/api/address/:villageId', authenticateApiKey, logRequest, async (req: Request, res: Response) => {
  try {
    const { villageId } = req.params;

    const result = await pool.query(
      `SELECT * FROM v_complete_addresses WHERE village_id = $1`,
      [villageId]
    );
...
app.get('/api/autocomplete', authenticateApiKey, logRequest, async (req: Request, res: Response) => {
  try {
    const query = (req.query.q as string)?.toLowerCase().trim();
    const type = (req.query.type as string) || 'village'; // village, district, state
    ...
    switch (type) {
      ...
      case 'district':
        result = await pool.query(
          `SELECT d.id, d.district_name as name, d.district_code as code, s.state_name
           FROM districts d
           JOIN states s ON d.state_id = s.id
